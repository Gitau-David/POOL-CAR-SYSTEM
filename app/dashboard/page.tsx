...
export default function DashboardPage() {
  ...
  const busyVehicleIds = new Set(
    requisitions.filter((r) => r.vehicle_id && (r.status === "Scheduled" || r.status === "Active") && r.start_date <= today && r.end_date >= today).map((r) => r.vehicle_id)
  );
  const busyDriverIds = new Set(
    requisitions.filter((r) => r.driver_id && (r.status === "Scheduled" || r.status === "Active") && r.start_date <= today && r.end_date >= today).map((r) => r.driver_id)
  );
